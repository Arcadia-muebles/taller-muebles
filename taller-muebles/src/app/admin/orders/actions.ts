...
export async function createOrder(
  ...
  const parsed = orderSchema.safeParse({
    ...
    customerAddress: formData.get("customerAddress")?.toString() || undefined,
    customerCommune: formData.get("customerCommune")?.toString() || undefined,
...
export async function moveOrderStage(input: z.infer<typeof moveOrderStageSchema>): Promise<MoveOrderStageResult> {
  ...
  if (isIndependentStartStep(parsed.data.stepKey)) {
    return { ok: false, message: "Estructura, Corte y Costura se actualizan individualmente desde su propio control." };
  }
  if (!hasSupabaseConfig()) {
